      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            {/* Dark Menu Panel - bg-slate-950 for dark theme */}
            <motion.div
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
              ...
              className="absolute right-0 top-0 h-full w-full max-w-sm bg-slate-950 border-l border-white/10 shadow-2xl"
            >
              {/* Menu Header - REMOVED duplicate close button, only shows "Menu" text */}
              <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-white/10">
                <span className="text-sm font-semibold text-white">Menu</span>
                {/* Close button REMOVED from here - now using the toggle button outside */}
              </div>
