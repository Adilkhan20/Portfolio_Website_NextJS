...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        <Container className="flex items-center justify-between h-14 sm:h-16 lg:h-20">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            {/* Mobile Menu Toggle - This is the ONLY working close button */}
            <button
      ...
      {/* MOBILE MENU OUTSIDE HEADER - To fix height/clipping issue */}
      <AnimatePresence>
        {isOpen && (
          <div className="fixed inset-0 z-[50] lg:hidden">
            {/* INVISIBLE OVERLAY - Sirf click catch karne ke liye, koi dark color nahi */}
            <div
              className="absolute inset-0 bg-transparent"
              onClick={() => setIsOpen(false)}
            />

            {/* DARK MENU PANEL (Half screen drawer) */}
            <motion.div
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
              transition={{ type: "spring", damping: 25, stiffness: 200 }}
              className="absolute right-0 top-0 h-[100dvh] w-full max-w-sm bg-slate-950 border-l border-white/10 shadow-2xl flex flex-col pt-24 pb-8 overflow-y-auto"
            >
              <div className="flex flex-col h-full px-6">
                {/* Menu Header (Title Only) */}
                <div className="flex items-center justify-between pl-12 pb-4 border-b border-white/10 size-12">
                  <span className="text-lg font-semibold text-white size-10">Menu</span>
                </div>
                ...
                <div className="mt-auto space-y-6 pt-6">
                  ...
                  <ButtonLink
                    href={`mailto:${siteConfig.email}`}
                    className="w-full bg-gradient-to-r from-teal-400 to-blue-500 hover:from-teal-500 hover:to-blue-600 text-white border-0 shadow-lg shadow-teal-500/20 flex justify-center py-3 rounded-xl"
                  >
                  ...
                  <div className="text-center">
                    <p className="text-xs text-slate-500">
                      Available for freelance &bull; {siteConfig.location}
                    </p>
                  </div>
                </div>
              </div>
